=== Reenable Shortlink Item In Admin Toolbar  ===
Contributors: klagraff
Donate link: 
Tags: shortlink, admin menu, 
Requires at least: 3.3
Tested up to: 3.4.2
Stable tag: 1.1

This plugin will display the shortlink menu item in the admin toolbar.

== Description ==

This plugin enables the short link menu item to display in the admin toolbar. This feature was removed from
version 3.3 by default.

== Installation ==

1. Upload this plugin directory to `/wp-content/plugins/` 
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Activate permalinks in the settings menu


== Screenshots ==

1. screenshot-1.png

== Changelog ==

= 1.0 =
* First release (December 13, 2011)

= 1.1 =
* Fixed visibility of shortlink 
* Added screenshot
* Added banner image